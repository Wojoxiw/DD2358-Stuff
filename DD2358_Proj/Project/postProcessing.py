...
def testSVD(problem): ## takes a problem, does test svd on it, using DASK
    ## load in all the data
    data = np.load(problem+'output.npz')
    b = data['b']
    fvec = data['fvec']
    S_ref = data['S_ref']
    S_dut = data['S_dut']
    ...
    with dolfinx.io.XDMFFile(MPI.COMM_WORLD, problem+'output-qs.xdmf', 'r') as f:
        mesh = f.read_mesh()
    ...
    with h5py.File(problem+'output-qs.h5', 'r') as f:
    ...
    with dolfinx.io.XDMFFile(MPI.COMM_WORLD, problem+'testoutput.xdmf', 'w') as f:
        f.write_mesh(mesh)
