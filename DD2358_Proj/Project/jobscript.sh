...
#SBATCH -N 8 ##number of nodes that will be allocated - must use --ntasks-per-node or --cpus-per-task to use more than 1 core per node
#SBATCH --tasks-per-node=3 ##number of cores used per task? - up to 48 per node for COSMOS. Presumably this is what I want, using MPI
...
#SBATCH -J 8nodes24MPI1threads2b ##puts a job name, to identify it
...
time mpirun -n 3 python runScatt3D.py ## run the main process, and time it
